...
class OrderManager:
    ...
    def is_market_hours(self) -> bool:
        ...
        Returns:
            True if within allowed trading window
        """
    ...
    async def execute_exit(
        ...
        # Market hours check
        if not self.is_market_hours():
            result['error'] = "Outside allowed trading hours (9:35-15:45 EST)"
            logger.warning(f"EXIT {symbol} rejected: {result['error']}")
            return result

        # Race condition check: verify position still exists at broker
        try:
