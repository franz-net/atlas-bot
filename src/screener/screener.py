...
class StockScreener:
    ...
    # Near-miss margins — only log rejections within these distances of the
    # gate boundary. Prevents the shadow log from filling with obvious rejects
    # (e.g. penny stocks, dead tickers). Tuned by gate sensitivity.
    NEAR_MISS_RSI_MARGIN = 10          # RSI points beyond min/max
    NEAR_MISS_RELVOL_MARGIN = 0.5      # relvol ratio below the min
    NEAR_MISS_ATR_MARGIN = 0.3         # atr_pct below the min
    NEAR_MISS_SPREAD_MULT = 2.0        # spread up to 2x the max

    def __init__(self, client: TradeStationClient, ledger=None):
        """
        Initialize the screener.

        Args:
            client: Authenticated TradeStationClient instance
            ledger: Optional TradingLedger for near-miss shadow logging.
                    If None, near-miss logging is a no-op.
        """
        self._client = client
        self._ledger = ledger
    ...
    def _log_near_miss(
        ...
        """
        Log a near-miss rejection to the ledger if one is configured.

        Silent no-op when no ledger is wired up (e.g. in unit tests).
        """
    ...
    def apply_price_spread_filter(
        ...
        for symbol, quote in quotes.items():
            ...
            if price < SCREENER_PRICE_MIN or price > SCREENER_PRICE_MAX:
                # Price-band failures aren't interesting near-misses — the
                # symbol universe is the universe. Skip logging.
                continue

            spread = calculate_spread_pct(bid, ask)
            if spread is None or spread > SCREENER_MAX_SPREAD_PCT:
                # Log a spread near-miss when the symbol is in the price band
                # but the spread is only moderately wide (<=2x the limit).
                if (spread is not None
                        and spread <= SCREENER_MAX_SPREAD_PCT * self.NEAR_MISS_SPREAD_MULT):
                    self._log_near_miss(
                        symbol=symbol,
                        gate='spread',
                        gate_detail=f'spread={spread:.3f}%, max={SCREENER_MAX_SPREAD_PCT}%',
                        entry_price=price,
                        metrics={'price': price, 'bid': bid, 'ask': ask, 'spread_pct': spread},
                    )
                continue
        ...
        for symbol in symbols:
            ...
            if rel_vol is None or rel_vol < SCREENER_MIN_RELATIVE_VOLUME:
                if (rel_vol is not None
                        and rel_vol >= SCREENER_MIN_RELATIVE_VOLUME - self.NEAR_MISS_RELVOL_MARGIN):
                    self._log_near_miss(
                        symbol=symbol,
                        gate='relvol',
                        gate_detail=f'relvol={rel_vol:.2f}, min={SCREENER_MIN_RELATIVE_VOLUME}',
                        entry_price=price,
                        metrics={'relative_volume': rel_vol, 'rsi': rsi, 'atr_pct': atr_pct},
                    )
                continue
            if atr_pct is None or atr_pct < SCREENER_MIN_ATR_PCT:
                if (atr_pct is not None
                        and atr_pct >= SCREENER_MIN_ATR_PCT - self.NEAR_MISS_ATR_MARGIN):
                    self._log_near_miss(
                        symbol=symbol,
                        gate='atr',
                        gate_detail=f'atr_pct={atr_pct:.2f}, min={SCREENER_MIN_ATR_PCT}',
                        entry_price=price,
                        metrics={'relative_volume': rel_vol, 'rsi': rsi, 'atr_pct': atr_pct},
                    )
                continue
            if rsi is None or rsi < SCREENER_RSI_MIN or rsi > SCREENER_RSI_MAX:
                if rsi is not None:
                    near_low = rsi < SCREENER_RSI_MIN and rsi >= SCREENER_RSI_MIN - self.NEAR_MISS_RSI_MARGIN
                    near_high = rsi > SCREENER_RSI_MAX and rsi <= SCREENER_RSI_MAX + self.NEAR_MISS_RSI_MARGIN
                    if near_low or near_high:
                        # RSI below range → would have been a long setup had it
                        # been less oversold; above range → would have been short.
                        implied_dir = 'LONG' if near_low else 'SHORT'
                        self._log_near_miss(
                            symbol=symbol,
                            gate='rsi',
                            gate_detail=f'rsi={rsi:.1f}, band=[{SCREENER_RSI_MIN}, {SCREENER_RSI_MAX}]',
                            entry_price=price,
                            would_be_direction=implied_dir,
                            metrics={'rsi': rsi, 'relative_volume': rel_vol, 'atr_pct': atr_pct},
                        )
                continue
        ...
        for candidate in candidates:
            ...
            if vwap_dist is not None and vwap_dist > SCREENER_MAX_VWAP_DISTANCE_PCT:
                self._log_near_miss(
                    symbol=symbol,
                    gate='vwap_distance',
                    gate_detail=f'vwap_dist={vwap_dist:.2f}%, max={SCREENER_MAX_VWAP_DISTANCE_PCT}%',
                    entry_price=price,
                    metrics={
            ...
            if regime == 'AVOID':
                ...
                self._log_near_miss(
                    symbol=symbol,
                    gate='regime',
                    gate_detail='regime=AVOID (no clear setup)',
                    entry_price=price,
                    metrics={
                        'rsi': rsi,
                        'vwap': vwap,
