...
class TestTradeDecisionSchema:
    ...
    As of v1.4, the AI no longer proposes SL/TP — Python computes them
    from ATR. These tests verify the reduced schema the AI still owns:
    direction, entry price, conviction, reasoning.
    """
    ...
    def test_missing_required_field(self):
        with pytest.raises(ValidationError):
            TradeDecision(
                action='ENTER',
                symbol='NVDA',
                # missing direction, expected_entry_price, conviction
    ...
    def test_low_conviction_rejected(self):
        """Conviction < MIN_CONVICTION_SCORE (7) is rejected as 'Not a high-quality setup'."""
        with pytest.raises(ValidationError, match='Conviction too low'):
            TradeDecision(
                action='ENTER',
                symbol='NVDA',
                direction='LONG',
                expected_entry_price=875.0,
                conviction=5,  # below MIN_CONVICTION_SCORE
                reasoning='Breaking above 20-day high on 2.3x relative volume with catalyst. RSI 58.',
            )

    def test_legacy_sltp_fields_ignored(self):
        """Older AI responses may still include stop_loss/take_profit — silently ignored."""
        trade = TradeDecision(
            action='ENTER',
            symbol='NVDA',
            direction='LONG',
            expected_entry_price=875.0,
            conviction=8,
            reasoning='Breaking above 20-day high on 2.3x relative volume with Benzinga upgrade. RSI 58 rising.',
            stop_loss=870.0,       # legacy — not on schema
            take_profit=900.0,     # legacy — not on schema
        )
        assert trade.symbol == 'NVDA'
        assert not hasattr(trade, 'stop_loss')
        assert not hasattr(trade, 'take_profit')

    def test_conviction_at_minimum_passes(self):
        """Conviction exactly at MIN_CONVICTION_SCORE (7) should pass."""
        trade = TradeDecision(
            action='ENTER',
            symbol='NVDA',
            direction='LONG',
...
class TestShareSizing:
    """Test Python-side share calculation + ATR bracket in _trade_to_dict."""
    ...
    def _trade(self, **kw):
        defaults = dict(
            action='ENTER', symbol='NVDA', direction='LONG',
            expected_entry_price=100.0, conviction=8,
    ...
    def test_share_sizing_expensive_stock_with_enough_capital(self, engine):
        ...
        result = engine._trade_to_dict(trade, active_capital=5000, atr_pct=2.0)
        assert result is not None
        assert result['shares'] == 1

    def test_share_sizing_cheap_stock(self, engine):
        trade = self._trade(symbol='F', expected_entry_price=10.0)
    ...
    def test_share_sizing_respects_max_pct(self, engine):
        ...
        result = engine._trade_to_dict(trade, active_capital=100, atr_pct=2.0)
        assert result is not None
        assert result['shares'] == 2
        assert result['shares'] * 10 <= 100 * 0.30

    def test_trade_dict_has_all_fields(self, engine):
        trade = self._trade(symbol='AAPL', expected_entry_price=150.0)
        result = engine._trade_to_dict(trade, active_capital=1000, atr_pct=2.0)
        assert result is not None
        for key in ('action', 'symbol', 'direction', 'shares', 'order_type',
                    'expected_entry_price', 'stop_loss', 'take_profit', 'reasoning'):
            assert key in result

    # ─── ATR-based bracket computation (v1.4) ──────────────────────

    def test_long_bracket_from_atr(self, engine):
        """LONG: SL = 1× ATR below entry, TP = 2.5× ATR above entry."""
        trade = self._trade(expected_entry_price=100.0, direction='LONG')
        # atr_pct=2.0 → SL dist = 2.0%, TP dist = 2.5 × 2.0 = 5.0%
        # SL = 100 × (1 - 0.02) = 98.00
        # TP = 100 × (1 + 0.05) = 105.00
        result = engine._trade_to_dict(trade, active_capital=1000, atr_pct=2.0)
        assert result['stop_loss'] == 98.00
        assert result['take_profit'] == 105.00

    def test_short_bracket_from_atr(self, engine):
        """SHORT: SL above entry, TP below entry (mirror of LONG)."""
        trade = self._trade(
            symbol='TSLA', direction='SHORT', expected_entry_price=100.0,
            reasoning='Breaking below VWAP with RSI 30 on 2.3x relative volume. Analyst downgrade catalyst confirmed.',
        )
        result = engine._trade_to_dict(trade, active_capital=1000, atr_pct=2.0)
        assert result['stop_loss'] == 102.00   # +2.0%
        assert result['take_profit'] == 95.00  # -5.0%

    def test_bracket_respects_min_sl_distance(self, engine):
        """Very low ATR → SL clamped to MIN_SL_DISTANCE_PCT (0.5%)."""
        trade = self._trade(expected_entry_price=100.0)
        # atr_pct=0.2 → 1× atr = 0.2%, clamped up to 0.5%
        result = engine._trade_to_dict(trade, active_capital=1000, atr_pct=0.2)
        # SL = 100 × (1 - 0.005) = 99.50
        assert result['stop_loss'] == 99.50
        # TP = 2.5 × 0.5% = 1.25% → 101.25, but MIN_TP = 1.25% → 101.25
        assert result['take_profit'] == 101.25

    def test_bracket_respects_max_sl_distance(self, engine):
        """Very high ATR → SL clamped to MAX_SL_DISTANCE_PCT (3.5%)."""
        trade = self._trade(expected_entry_price=100.0)
        # atr_pct=10.0 → 1× atr = 10%, clamped down to 3.5%
        result = engine._trade_to_dict(trade, active_capital=1000, atr_pct=10.0)
        # SL = 100 × (1 - 0.035) = 96.50
        assert result['stop_loss'] == 96.50
        # TP = 2.5 × 3.5% = 8.75% → 108.75
        assert result['take_profit'] == 108.75

    def test_bracket_with_missing_atr_uses_min(self, engine):
        """If candidate has no ATR data, fall back to MIN_SL_DISTANCE_PCT."""
        trade = self._trade(expected_entry_price=100.0)
        result = engine._trade_to_dict(trade, active_capital=1000, atr_pct=0)
        # SL = 100 × (1 - 0.005) = 99.50 (MIN_SL=0.5%)
        assert result['stop_loss'] == 99.50

    def test_bracket_ignores_ai_proposed_sltp(self, engine):
        """Even if AI somehow sent stop_loss/take_profit, Python's ATR values win."""
        trade = self._trade(
            expected_entry_price=100.0,
            # These are accepted by extra='ignore' but shouldn't affect output
            stop_loss=50.0, take_profit=200.0,
        )
        result = engine._trade_to_dict(trade, active_capital=1000, atr_pct=2.0)
        # Still computed from ATR, not from AI's fantasy numbers
        assert result['stop_loss'] == 98.00
        assert result['take_profit'] == 105.00
...
class TestDecisionCycle:
    ...
    async def test_enter_decision(self, engine, mock_provider, sample_candidates, sample_account_state):
        ...
        # SL/TP computed by Python from candidate atr_pct=2.1:
        # SL dist = 2.1%, TP dist = 2.5 × 2.1 = 5.25%
        # SL = 875 × (1 - 0.021) = 856.625 → rounded to 856.62 or 856.63
        assert abs(result['trades'][0]['stop_loss'] - 856.63) < 0.01
        assert abs(result['trades'][0]['take_profit'] - 920.94) < 0.01
    ...
    @pytest.mark.asyncio
    async def test_low_conviction_rejected_via_pydantic(self, engine, mock_provider, sample_candidates, sample_account_state):
        """AI returns conviction below threshold — Pydantic rejects the whole response."""
        mock_provider.decide.return_value = {
            'success': True,
            'content': json.dumps({
                'action': 'ENTER',
                'trades': [{
                    'action': 'ENTER', 'symbol': 'NVDA', 'direction': 'LONG',
                    'expected_entry_price': 875.0,
                    'conviction': 4,  # below MIN_CONVICTION_SCORE
