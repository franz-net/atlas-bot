...
class TestExitExecution:
    ...
    async def test_exit_success(self, manager, mock_client, sample_exit_decision, sample_open_trade):
        ...
        with patch.object(manager, 'is_exchange_open', return_value=True):
    ...
    @pytest.mark.asyncio
    async def test_exit_rejected_when_exchange_closed(
        self, manager, mock_client, sample_exit_decision, sample_open_trade
    ):
        """With an open position, EXIT is rejected only when the exchange itself is closed."""
        mock_client.get_positions.return_value = [
            {'Symbol': 'PATH', 'Quantity': 86},
        ]
        with patch.object(manager, 'is_exchange_open', return_value=False):
            result = await manager.execute_exit(sample_exit_decision, sample_open_trade)

        assert result['success'] is False
        assert 'Exchange closed' in result['error']

    @pytest.mark.asyncio
    async def test_exit_allowed_in_last_15min_window(
        self, manager, mock_client, sample_exit_decision, sample_open_trade
    ):
        """Regression for the HAL 15:45 EDT incident: the entry-hours
        window (ends 15:45) must NOT block exits when the exchange is
        still open. Exit path uses is_exchange_open (ends 15:58)."""
        mock_client.get_positions.return_value = [
            {'Symbol': 'PATH', 'Quantity': 86},
        ]
        mock_client.cancel_order = AsyncMock(return_value=True)
        ...
        # Simulate 15:50 EDT: entry window closed, exchange window open.
        with patch.object(manager, 'is_market_hours', return_value=False), \
             patch.object(manager, 'is_exchange_open', return_value=True):
            result = await manager.execute_exit(sample_exit_decision, sample_open_trade)

        assert result['success'] is True

    @pytest.mark.asyncio
    async def test_exit_no_position_returns_already_closed_regardless_of_hours(
        self, manager, mock_client, sample_exit_decision, sample_open_trade
    ):
        """If broker reports no position, the time check is irrelevant — nothing to close."""
        mock_client.get_positions.return_value = []

        # Even with both time windows closed, 'already closed' is returned
        # rather than a time-window rejection.
        with patch.object(manager, 'is_exchange_open', return_value=False), \
             patch.object(manager, 'is_market_hours', return_value=False):
            result = await manager.execute_exit(sample_exit_decision, sample_open_trade)
