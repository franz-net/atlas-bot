...
class TradingScheduler:
    ...
    def __init__(self):
        ...
        self._discord: DiscordNotifier = DiscordNotifier()
    ...
    async def _get_account_state(self) -> Optional[dict]:
        ...
        try:
            ...
            # Equity includes unrealized P&L from open positions — used for
            # position sizing so we don't over-leverage concurrent positions.
            equity = float(balance_data.get('Equity', balance_data.get('CashBalance', 0)))

            # Cash balance only changes on realized fills — used for daily
            # snapshots and P&L tracking so numbers don't fluctuate with
            # open-position mark-to-market.
            cash_balance = float(balance_data.get('CashBalance', equity))
    ...
    async def run_cycle(self) -> dict:
        ...
        try:
            ...
            # Use cash_balance (realized only) so P&L tracking isn't polluted
            # by unrealized mark-to-market on open positions.
            today = datetime.now(est).strftime('%Y-%m-%d')
            sod_active = account_state['cash_balance'] - PROTECTED_FLOOR
            self._ledger.record_daily_snapshot(
                today, sod_active, account_state['cash_balance'], self._mode,
            )
            ...
            if active_capital < hard_stop_value:
                ...
                await self._discord.notify_error(
                    'HARD_STOP',
                    f"Active capital ${active_capital:.2f} below "
                    f"{HARD_STOP_ACTIVE_CAPITAL_PCT*100:.0f}% of start-of-day "
                    f"(${hard_stop_value:.2f}). Trading paused.",
                    mode=self._mode,
                )
                ...
                if approved_trades:
                    ...
                    for exec_result in exec_results:
                        if exec_result['success']:
                            ...
                            # Snapshot the headlines the AI saw for this symbol
                            # at entry time so the audit trail shows which
                            # catalyst (if any) drove the trade.
                            symbol_news = news_data.get(exec_result['symbol'], [])
                            news_catalyst_json = (
                                json.dumps(symbol_news) if symbol_news else None
                            )
                            self._ledger.record_trade_entry(
                                ...
                                news_catalyst=news_catalyst_json,
                            ...
                            await self._discord.notify_entry(
                                symbol=exec_result['symbol'],
                                direction=exec_result['direction'],
                                shares=exec_result['shares'],
                                entry_price=exec_result.get('entry_price', 0),
                                ...
                                conviction=trade_decision.get('conviction', 0),
                                mode=self._mode,
                            )
                        else:
                            logger.warning(
                            ...
                            await self._discord.notify_error(
                                'TRADE_FAILED',
                                f"{exec_result['symbol']}: {exec_result['error']}",
                                mode=self._mode,
                            )
                ...
                if approved_exits:
                    ...
                    for exit_result in exit_results:
                        if exit_result.get('success'):
                            ...
                            await self._discord.notify_exit(
                                symbol=exit_result['symbol'],
                                direction=direction,
                                pnl_dollars=pnl_dollars,
                                pnl_pct=pnl_pct,
                                exit_reason=f"AI EXIT: {exit_result.get('reasoning', '')[:200]}",
                                mode=self._mode,
                            )
                            ...
                            await self._discord.notify_error(
                                'EXIT_FAILED',
                                f"{exit_result['symbol']}: {exit_result.get('error', 'unknown')}",
                                mode=self._mode,
                            )
            ...
            await self._discord.notify_error(
                'CYCLE_ERROR',
                f"Cycle {self._cycle_count} failed: {e}",
                mode=self._mode,
            )
    ...
    async def start(self) -> None:
        ...
        async with self._client:
            # Initialize components (ledger first so screener can log near-misses)
            self._ledger = TradingLedger()
            self._screener = StockScreener(self._client, ledger=self._ledger)
            self._builder = CandidateBuilder()
            self._news = NewsFetcher(self._client)
            self._engine = DecisionEngine(ledger=self._ledger)
            ...
            while self._running:
                try:
                    if self.is_market_open():
                        ...
                        # Send daily summary once after market close
                        today_str = now.strftime('%Y-%m-%d')
                        if (now.hour >= MARKET_CLOSE_HOUR
                                and self._ledger.get_state('daily_summary_date') != today_str):
                            self._ledger.set_state('daily_summary_date', today_str)
                            await self._send_daily_summary(today_str)

                        # Backfill forward returns on shadow-logged near-misses
                        if (now.hour >= MARKET_CLOSE_HOUR
                                and self._ledger.get_state('near_miss_backfill_date') != today_str):
                            self._ledger.set_state('near_miss_backfill_date', today_str)
                            await self._run_near_miss_backfill()

                        # Run weekly review on Fridays after market close
                        if (now.weekday() == 4
                                and now.hour >= MARKET_CLOSE_HOUR
                                and self._ledger.get_state('eod_review_date') != today_str):
                            await self._run_eod_tasks(now)

                        # No midnight reset needed — date comparison handles day rollover
        ...
        while self._running:
            try:
                ...
                for exit_info in exits:
                    ...
                    await self._discord.notify_exit(
                        symbol=symbol,
                        direction=exit_info.get('direction', 'LONG'),
                        pnl_dollars=pnl,
    ...
    async def _send_daily_summary(self, date: str) -> None:
        """
        Send end-of-day Discord summary with today's trades and P&L.

        Args:
            date: Date string (YYYY-MM-DD)
        """
        try:
            trades_today = self._ledger.get_trades_closed_on_date(date)
            daily_pnl = sum(t.get('pnl_dollars', 0) for t in trades_today)

            all_closed = self._ledger.get_closed_trades()
            total_pnl = sum(
                t.get('pnl_dollars', 0) - t.get('entry_fees', 0) - t.get('exit_fees', 0)
                for t in all_closed
            )

            account_state = await self._get_account_state()
            balance = account_state['cash_balance'] if account_state else 0
            active = account_state['active_capital'] if account_state else 0

            await self._discord.notify_daily_summary(
                date=date,
    ...
    async def _run_near_miss_backfill(self) -> None:
        """
        Run the near-miss outcome backfill once per day.

        Fetches forward returns for shadow-logged gate rejections so the
        Friday weekly review has outcome stats on what the gates filtered.
        Safe to skip on failure — runs again tomorrow.
        """
        try:
            from src.jobs.backfill_near_miss_outcomes import backfill_once
            result = await backfill_once(self._client, self._ledger)
            if result['updated'] > 0:
                logger.info(
                    f"Near-miss backfill: {result['updated']} updated, "
                    f"{result['skipped']} skipped"
                )
        except Exception as e:
            logger.error(f"Near-miss backfill failed: {e}")
    ...
    async def _run_eod_tasks(self, now: datetime) -> None:
        ...
        """
        self._ledger.set_state('eod_review_date', now.strftime('%Y-%m-%d'))
        ...
        try:
            ...
            if result.get('skipped'):
                ...
                await self._discord.notify_error(
                    'REVIEW_FAILED',
                    f"Weekly review failed: {result.get('reason', 'Unknown')}",
                    mode=self._mode,
                )
        except Exception as e:
