...
Market Open (every 5 min cycle)
  -> Stock Screener (150 symbols -> top 5 candidates)
  ...
  -> RSI Direction Gate (LONG needs RSI >= 55 or <= 35 reversal; SHORT needs RSI <= 45)
  -> Conviction Gate (AI self-rates 1-10, minimum 7 to enter)
...
- Conviction gate: AI must self-rate conviction 1-10, minimum 7 enforced in Pydantic schema
- RSI direction gate: LONG requires RSI >= 55 (momentum) or <= 35 (oversold reversal), SHORT requires RSI <= 45
- Screener filters: relative volume >= 2x, RSI 30-65, ATR >= 1.5%, top 5 candidates to AI
...
│   ├── ledger/
│   │   ├── ledger.py           # SQLite audit trail (7 tables)
...
```bash
pytest tests/ -v                # Run all 328 tests
```
